...
async function hubungkanKeWhatsApp(nomorHPTumbal = null, chatId = null, isRetry = false, forceNewSession = false) {
    ...
    if (forceNewSession) {
        ...
        // Menghapus file sesi lama di root jika force login baru
        if (fs.existsSync('creds.json')) {
            try { fs.unlinkSync('creds.json'); } catch(e){}
        }
        await delay(1000);
    }

    // MODIFIKASI AKHIR: Membaca sesi langsung dari root folder (tidak masuk folder sesi_wa)
    ...
    sock = makeWASocket({ 
        ...
        // Menyamar jadi Windows Desktop agar koneksi dari Railway lebih stabil & aman bypass
        ...
        if (connection === 'close') {
            ...
            if (statusCode === DisconnectReason.loggedOut) {
                ...
                if (chatId) {
                    ...
                    teksAlert += `🛠️ **Tindakan Otomatis:** Sistem telah mendepak dan menghapus berkas sesi.\n\n`;
                ...
                if (state.creds && state.creds.me && state.creds.me.id) {
                    console.log(`[SYSTEM] Sesi terputus temporer (Code: ${statusCode || 'Transisi'}). Mencoba hubungkan ulang dalam 5 detik...`);
                    await delay(5000); 
...
bot.on('callback_query', (query) => {
    ...
    if (query.data === 'sambungkan') {
        bot.sendMessage(chatId, "📟 **[INPUT REQUIRED]**\nMasukkan nomor WhatsApp target pairing (Contoh: 08xxxx or 628xxxx):", { reply_markup: { force_reply: true } });
...
bot.on('message', async (msg) => {
    ...
    if (msg.text.match(/^\/cek(\s|$)/) && !msg.reply_to_message) {
        ...
        for (let item of daftarNomor) {
            ...
            if (counter % 5 === 0 || counter === totalNomor) {
                let hasilTeks = `🔮 ── **𝕄𝕌𝕃𝕋𝕀-𝕊ℂ𝔸ℕ ℝ𝔼𝕊𝕌𝕃𝕋** ── 🔮\n\n`;
